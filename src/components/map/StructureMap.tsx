 ...
 *
 * `focusLocation` flies the camera to a specific coordinate and pops its
 * marker open once that structure's marker exists — used right after
 * registering a new structure, so the map doesn't just silently grow a new
 * pin somewhere off-screen while the camera stays wherever it was.
 *
 * THE MAP INSTANCE IS CREATED EXACTLY ONCE (per mount) and never torn down
 * just because `structures`/`zones`/`view` changed. An earlier version
 * rebuilt the whole `mapboxgl.Map` from scratch on every data change —
 * registering a structure invalidates the structures query, which changes
 * the `structures` array reference, which recreated the map mid-flight and
 * cancelled the very flyTo animation meant to show where the new structure
 * landed. Markers, the heatmap layer, and the camera are now updated
 * imperatively against the *same* map instance instead.
 */
export function StructureMap({
  structures,
  zones,
  ...
  // Flips once the map's own "load" event has fired. Markers/heatmap/camera
  // effects all wait on this instead of racing the async mapbox-gl import.
  ...
  useEffect(() => {
    ...
    // Intentionally just `token`: this effect must run once per mount, not
    // on every structures/zones/view update — see the function doc comment.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token]);

  // --- Keep markers in sync with `structures` (and risk-dependent styling
  //     from `view`), without touching the map instance itself. -----------
  useEffect(() => {
    ...
    import("mapbox-gl").then((mapboxgl) => {
      if (mapRef.current !== map) return; // map was torn down mid-import
      ...
      // First time markers ever land and no explicit focus was requested,
      // frame all of them instead of sitting at the arbitrary [-122.4,
      // 37.77] fallback center used before any data existed.
      if (!focusLocation && structures.length > 0) {
        const lngs = structures.map((s) => s.lng);
        const lats = structures.map((s) => s.lat);
        map.fitBounds(
          [
  ...
  }, [structures, view, mapReady, focusLocation]);

  // --- Heatmap source/layer, updated in place rather than re-added. ------
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReady) return;
    ...
    const existingSource = map.getSource(HEATMAP_SOURCE_ID) as import("mapbox-gl").GeoJSONSource | undefined;
    if (existingSource) {
    ...
    if (map.getLayer(HEATMAP_LAYER_ID)) {
      map.setPaintProperty(HEATMAP_LAYER_ID, "heatmap-opacity", view === "heatmap" ? 0.85 : 0);
    }
  }, [zones, view, mapReady]);

  // --- Fly to a newly-registered structure and pop its marker open. ------
  useEffect(() => {
    const map = mapRef.current;
    ...
    if (match) {
      const index = structures.indexOf(match);
      markersRef.current[index]?.togglePopup();
    }
  }, [focusLocation, mapReady, structures]);
